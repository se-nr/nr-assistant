/**
 * OAuth Callback — Server-side Supabase PKCE code exchange
 *
 * GET /api/auth/callback?code={supabase_pkce_code}
 *
 * After Google OAuth, Supabase redirects here with a PKCE auth code.
 * We exchange it for a Supabase session using the code_verifier stored
 * in the mcp_auth_state cookie (set by /authorize), then redirect to
 * /api/auth/complete with the Supabase access token.
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Read Supabase PKCE code from query params
  ...
  // Exchange Supabase PKCE code for session
  try {
    ...
    if (!tokenRes.ok) {
      ...
      errorPage(
        ...
        "Could not verify your Google login. Please try again."
      );
      return;
    }
