...
import { getEmailMetrics, getSubscriberStats, getFlowsWithPerformance, getCampaignsWithPerformance, getRevenueAttribution, listLists, listSegments, listMetrics, validateApiKey, } from "./klaviyo-client.js";
// ─── Supabase setup ─────────────────────────────────────────────────────────
...
export function createMcpServer() {
    ...
    server.prompt("nr-agency-guide", "Komplet guide til N+R Agency MCP — tool-oversigt, workflows og regler. Kald dette FØRST i enhver samtale.", {}, async () => ({
        messages: [{
                role: "user",
                content: {
                    type: "text",
...
"Sammenlign perioder" → get_clients → compare_periods`
    ...
    // ─── Tool: get_clients ──────────────────────────────────────────────────────
    server.tool("get_clients", "List alle aktive klienter med id, navn og platform-info", {}, async () => {
        ...
        const { data, error } = await sb
            ...
            .order("name");
        if (error)
            return { content: [{ type: "text", text: `Fejl: ${error.message}` }] };
        const list = (data || [])
            .map((c) => [
    ...
    server.tool("get_performance", "Hent aggregeret performance for en klient (spend, ROAS, purchases, reach). Kald get_brand_context først for at forstå klientens brand, målgruppe og strategi.", {
        ...
        if (!client)
            return noClient(client_name);
        const { since, until } = resolveDateRange(time_range);
        try {
            const data = await dashboardFetch("/api/dashboard/insights", {
                clientId: client.id,
                from: since,
                to: until,
                comparison: "none",
            ...
            if (!s)
                return noData(client.name);
            const channelLines = (data.channelBreakdown || []).map((ch) => `| ${ch.channel} | ${formatCurrency(ch.spend)} | ${ch.roas?.toFixed(2) || "–"}x | ${ch.purchases || 0} |`);
            const report = [
    ...
    // ─── Tool: get_top_ads ──────────────────────────────────────────────────────
    server.tool("get_top_ads", "Top-performende annoncer for en klient sorteret efter ROAS eller spend. Kald get_brand_context først for at forstå klientens kreative strategi.", {
        ...
        const { since, until } = resolveDateRange(time_range);
        try {
            const data = await dashboardFetch("/api/internal/top-ads", {
                clientId: client.id,
                from: since,
                to: until,
                sort_by,
            ...
            const ads = data.ads || [];
            if (!ads.length)
                return noData(client.name, "annonce");
            const lines = [
    ...
    server.tool("get_client_documents", "Hent klientdokumenter fra Supabase (strategier, briefs, research, rapporter, overview). Filtrér valgfrit på doc_type.", {
        ...
        if (error)
            return { content: [{ type: "text", text: `Fejl: ${error.message}` }] };
        if (!docs?.length) {
            return { content: [{ type: "text", text: `Ingen dokumenter fundet for ${client.name}${doc_type !== "all" ? ` (type: ${doc_type})` : ""}` }] };
        }
    ...
    server.tool("save_client_document", "Gem et dokument for en klient i Supabase (strategi, brief, research, rapport, overview). Opdaterer eksisterende hvis samme title+doc_type findes.", {
        client_name: z.string().describe("Klientens navn"),
        ...
        if (existing?.length) {
            ...
            const { error } = await sb
                ...
                .eq("id", existing[0].id);
            if (error)
                return { content: [{ type: "text", text: `Fejl ved opdatering: ${error.message}` }] };
            return { content: [{ type: "text", text: `Opdateret: "${title}" (${doc_type}) for ${client.name}` }] };
        }
        // Opret nyt
        const { error } = await sb
            .from("client_documents")
        ...
        if (error)
            return { content: [{ type: "text", text: `Fejl ved oprettelse: ${error.message}` }] };
        return { content: [{ type: "text", text: `Gemt: "${title}" (${doc_type}) for ${client.name}` }] };
    });
    // ─── Tool: get_brand_context ──────────────────────────────────────────────
    server.tool("get_brand_context", "Hent KOMPLET brand-kontekst for en klient: profile (tone of voice, USP, målgruppe), brand research, VoC library, godkendt copy og strategi — PLUS N+R's Full Funnel metodik og analyse-frameworks. BRUG ALTID DETTE FØR analyse, content-produktion eller rådgivning for en klient.", {
        client_name: z.string().describe("Klientens navn (delvis match OK)"),
        ...
        if (include_agency_frameworks && client.name !== "Neble+Rohde") {
            ...
            if (nrClients?.length) {
                ...
                if (frameworks)
                    agencyDocs = frameworks;
            }
        }
        if (!docs?.length && !agencyDocs.length) {
            return { content: [{ type: "text", text: `Ingen brand-kontekst fundet for ${client.name}. Overvej at uploade profile, research og VoC via save_client_document.` }] };
        }
        // Gruppér klient-docs
        const groups = {};
        for (const d of (docs || [])) {
            if (!groups[d.doc_type])
                groups[d.doc_type] = [];
    ...
    server.tool("get_agency_knowledge", "Hent N+R's samlede metodik: Full Funnel strategi (FP→IM→IP→EC), Brand Lifecycle, Value Proposition Canvas, Cialdini's 7 principper, marketing psykologi, analyse-frameworks (Meta, Google, Klaviyo), copy-frameworks (email, Meta, Google) og strategi-framework. Brug dette til at forstå HVORDAN N+R analyserer, positionerer brands og producerer content.", {
        ...
        const { data: nrClients } = await sb
            ...
            .limit(1);
        if (!nrClients?.length) {
            return { content: [{ type: "text", text: "Neble+Rohde klient ikke fundet i Supabase." }] };
        }
        const titleMap = {
        ...
        if (error)
            return { content: [{ type: "text", text: `Fejl: ${error.message}` }] };
        if (!docs?.length) {
            return { content: [{ type: "text", text: `Ingen agency knowledge fundet${framework !== "all" ? ` for "${framework}"` : ""}.` }] };
        }
        const sections = [
    ...
    server.tool("get_demographic_breakdown", "Demografisk breakdown for en klient (alder, køn, platform, placement, device) med ROAS og spend", {
        ...
        const { since, until } = resolveDateRange(time_range);
        try {
            const data = await dashboardFetch("/api/internal/demographics", {
                clientId: client.id,
                from: since,
                to: until,
                breakdown,
            ...
            if (!rows.length)
                return noData(client.name, breakdown);
            const totalSpend = rows.reduce((s, r) => s + r.spend, 0);
            const label = { age: "Alder", gender: "Køn", platform: "Platform", placement: "Placement", device: "Device" };
            const lines = [
                `## ${client.name} – Breakdown: ${label[breakdown] || breakdown} (${time_range})`,
                ``,
                `| ${label[breakdown] || breakdown} | Spend | % | ROAS | CTR | Køb |`,
                `|------------|-------|---|------|-----|-----|`,
                ...rows.map((r) => `| ${r.dimension} | ${formatCurrency(r.spend)} | ${totalSpend > 0 ? (r.spend / totalSpend * 100).toFixed(1) : "0"}% | ${r.roas.toFixed(2)}x | ${r.ctr.toFixed(2)}% | ${Math.round(r.purchases)} |`),
            ];
            return text(lines.join("\n"));
        }
        catch (e) {
    ...
    // ─── Tool: trigger_sync ─────────────────────────────────────────────────────
    server.tool("trigger_sync", "Trigger en Meta Ads sync for en klient via Inngest (kræver dashboard-endpoint)", {
        client_name: z.string(),
    ...
    // ─── Tool: get_campaigns ──────────────────────────────────────────────────
    server.tool("get_campaigns", "Hent kampagner for en klient med status, objektiv og budget. Brug source til at filtrere på Meta eller Google Ads.", {
        client_name: z.string().describe("Klientens navn"),
        ...
        if (!data?.length)
            return text(`Ingen kampagner fundet for ${client.name}${source !== "all" ? ` (source: ${source})` : ""}`);
        const lines = [
    ...
    // ─── Tool: get_ad_sets ────────────────────────────────────────────────────
    server.tool("get_ad_sets", "Hent ad sets for en klient, valgfrit filtreret på kampagne. Brug source til at filtrere på Meta eller Google Ads.", {
        client_name: z.string().describe("Klientens navn"),
        ...
        if (error)
            return err(error.message);
        if (!data?.length)
            return text(`Ingen ad sets fundet for ${client.name}${source !== "all" ? ` (source: ${source})` : ""}`);
        const lines = [
    ...
    // ─── Tool: get_creatives ──────────────────────────────────────────────────
    server.tool("get_creatives", "Hent creatives for en klient med type, headline, body og CTA", {
        client_name: z.string().describe("Klientens navn"),
        ...
        if (error)
            return err(error.message);
        if (!data?.length)
            return text(`Ingen creatives fundet for ${client.name}`);
        const sections = data.map((c, i) => [
    ...
    server.tool("get_campaign_details", "Detaljeret kampagnevisning med ad sets og aggregeret performance", {
        client_name: z.string().describe("Klientens navn"),
        ...
        const { data: camps } = await sb
            ...
            .limit(1);
        if (!camps?.length)
            return text(`Ingen kampagne fundet: "${campaign_name}"`);
        const camp = camps[0];
        const { since, until } = resolveDateRange(time_range);
        ...
        try {
            const [adSetsResult, metricsData] = await Promise.all([
                ...
                dashboardFetch("/api/dashboard/insights", {
                    clientId: client.id,
                    from: since,
                    to: until,
                    campaignId: camp.id,
            ...
            if (s) {
                lines.push(`| Spend | ${formatCurrency(s.spend)} |`, `| Omsætning | ${formatCurrency(s.purchase_value)} |`, `| ROAS | ${Number(s.roas).toFixed(2)}x |`, `| Køb | ${Math.round(s.purchases)} |`, `| Klik | ${formatNum(s.clicks)} |`, `| CTR | ${Number(s.ctr).toFixed(2)}% |`, `| CPM | ${Math.round(s.cpm)} kr |`, `| CPA | ${s.purchases > 0 ? Math.round(s.cpa) + " kr" : "–"} |`, `| Reach | ${formatNum(s.reach)} |`);
            }
            else {
                lines.push(`| _Ingen data_ | – |`);
            }
            lines.push(``, `### Ad Sets (${adSets.length} stk)`, `| Ad Set | Status | Optimering | Budget |`, `|--------|--------|------------|--------|`, ...adSets.map((a) => `| \`${a.name}\` | ${a.status} | ${a.optimization_goal || "–"} | ${a.daily_budget ? formatCurrency(a.daily_budget) + "/dag" : "–"} |`));
            return text(lines.join("\n"));
        }
        catch (e) {
            return err(`Dashboard API: ${e.message}`);
    ...
    server.tool("compare_periods", "Sammenlign performance mellem to perioder (fx denne måned vs forrige). Kald get_brand_context først for klient-kontekst.", {
        client_name: z.string().describe("Klientens navn"),
        ...
        try {
            ...
            if (!a && !b)
                return noData(client.name);
            function val(s, key) { return s ? (Number(s[key]) || 0) : 0; }
    ...
    server.tool("get_country_breakdown", "Land-niveau performance fra demographic_insights (country-rækker) med spend, ROAS, køb", {
        client_name: z.string().describe("Klientens navn"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
        ...
        try {
            ...
            const rows = data.data || [];
            if (!rows.length)
                return noData(client.name, "land");
            const totalSpend = rows.reduce((s, r) => s + r.spend, 0);
            const lines = [
                ...
                `|------|-------|---|------|-----|------|`,
                ...rows.map((r) => `| ${r.dimension} | ${formatCurrency(r.spend)} | ${totalSpend > 0 ? (r.spend / totalSpend * 100).toFixed(1) : "0"}% | ${r.roas.toFixed(2)}x | ${Math.round(r.purchases)} | ${formatNum(r.clicks)} |`),
            ];
            return text(lines.join("\n"));
        }
        catch (e) {
    ...
    server.tool("get_lead_cohorts", "Hent lead cohort data: leads, konverteringsrate, revenue per lead, ROAS over tid. Bruger snapshots for hurtig aggregering.", {
        client_name: z.string().describe("Klientens navn"),
        ...
        try {
            ...
            if (cohort_month) {
                params.startMonth = cohort_month;
                params.endMonth = cohort_month;
            }
            const data = await dashboardFetch("/api/lead-cohorts", params);
            const cohorts = data.cohorts || [];
            if (!cohorts.length)
                return noData(client.name, "lead cohort");
            // Filter by ad_set_name if provided (client-side filter)
            const filteredCohorts = ad_set_name
                ? cohorts.map((c) => ({
                    ...c,
                    byAdSet: (c.byAdSet || []).filter((a) => a.adSetName?.toLowerCase().includes(ad_set_name.toLowerCase())),
                })).filter((c) => c.byAdSet.length > 0)
                : cohorts;
            ...
            if (ad_set_name) {
                ...
                const lines = [
                    ...
                    `|--------|--------|-------|-------|---------|----------|`,
                    ...filteredCohorts.flatMap((c) => c.byAdSet.map((a) => `| ${c.month} | \`${a.adSetName}\` | ${a.leads} | ${a.convRate?.toFixed(1) || "0"}% | ${formatCurrency(a.revenue || 0)} | ${formatCurrency(a.revPerLead || 0)} |`)),
                ];
                return text(lines.join("\n"));
            }
            ...
            const lines = [
                ...
                `|--------|-------|-------|----------|-----|---------|------|------------|`,
                ...cohorts.map((c) => `| ${c.month} | ${c.leadsCount} | ${((c.metrics?.find((m) => m.daysSinceLead === 90)?.convRate) || c.metrics?.[c.metrics.length - 1]?.convRate || 0).toFixed(1)}% | ${formatCurrency(c.adSpend || 0)} | ${formatCurrency(c.costPerLead || 0)} | ${formatCurrency(c.totalRevenue || 0)} | ${c.roas?.toFixed(2) || "–"}x | ${c.totalRoas?.toFixed(2) || "–"}x |`),
            ];
            return text(lines.join("\n"));
        }
        catch (e) {
    ...
    // ─── Tool: get_leads ──────────────────────────────────────────────────────
    server.tool("get_leads", "Hent individuelle leads fra lead_cohort_profiles med konverteringsstatus, revenue og kilde", {
        client_name: z.string().describe("Klientens navn"),
        ...
        let query = sb
            ...
            .limit(limit);
        if (cohort_month)
            query = query.eq("cohort_month", cohort_month);
        if (converted_only)
            query = query.not("first_order_date", "is", null);
        const { data, error } = await query;
        if (error)
            return err(error.message);
        if (!data?.length)
            return text(`Ingen leads fundet for ${client.name}`);
        const lines = [
            `## ${client.name} – Leads (${data.length} stk)${cohort_month ? ` – ${cohort_month}` : ""}`,
            ``,
            `| Lead dato | Email | Ad Set | Ordrer | Revenue | Dage til konv. |`,
            `|-----------|-------|--------|--------|---------|----------------|`,
            ...data.map((l) => `| ${l.lead_date} | ${l.email ? l.email.replace(/@.*/, "@…") : "–"} | \`${l.ad_set_name || "–"}\` | ${l.total_orders || 0} | ${formatCurrency(l.total_revenue || 0)} | ${l.days_to_convert ?? "–"} |`),
        ];
        return text(lines.join("\n"));
    });
    // ─── Tool: get_lead_orders ────────────────────────────────────────────────
    server.tool("get_lead_orders", "Hent ordrer fra leads med ordreværdi, dato og dage siden lead", {
        client_name: z.string().describe("Klientens navn"),
        ...
        let query = sb
            ...
            .limit(limit);
        if (days_max !== undefined)
            query = query.lte("days_since_lead", days_max);
        const { data, error } = await query;
        if (error)
            return err(error.message);
        if (!data?.length)
            return text(`Ingen lead-ordrer fundet for ${client.name}`);
        const totalValue = data.reduce((s, o) => s + (o.order_value || 0), 0);
        const lines = [
            ...
            `|------|----------|-------|-----------------|`,
            ...data.map((o) => `| ${o.order_date} | ${o.order_id} | ${formatCurrency(o.order_value)} | ${o.days_since_lead ?? "–"} |`),
        ];
        return text(lines.join("\n"));
    });
    // ─── Tool: get_hourly_data ────────────────────────────────────────────────
    server.tool("get_hourly_data", "Hent time-for-time performance (spend, klik, køb) – kun tilgængelig for de seneste 7 dage", {
        client_name: z.string().describe("Klientens navn"),
        ...
        if (error)
            return err(error.message);
        if (!data?.length)
            return noData(client.name, "time");
        ...
        const lines = [
            ...
            `|------|-------|------|-----|---------|`,
            ...data.map((r) => `| ${String(r.hour).padStart(2, "0")}:00 | ${formatCurrency(r.spend)} | ${r.clicks} | ${r.purchases} | ${formatCurrency(r.revenue)} |`),
        ];
        return text(lines.join("\n"));
    });
    // ─── Tool: get_targets ────────────────────────────────────────────────────
    server.tool("get_targets", "Hent performance-targets for en klient (ROAS, CTR, CPA mål) og sammenlign med aktuel performance", {
        client_name: z.string().describe("Klientens navn"),
    }, async ({ client_name }) => {
        const sb = getSupabase();
        const client = await findClient(sb, client_name);
        if (!client)
            return noClient(client_name);
        try {
            const data = await dashboardFetch("/api/internal/targets", { clientId: client.id });
            ...
            if (targets.roas)
                lines.push(`| ROAS | ${targets.roas}x | ${actual.roas.toFixed(2)}x | ${actual.roas >= targets.roas ? "✅" : "⚠️"} |`);
            if (targets.ctr)
                lines.push(`| CTR | ${targets.ctr}% | ${actual.ctr.toFixed(2)}% | ${actual.ctr >= targets.ctr ? "✅" : "⚠️"} |`);
            if (targets.cpa)
                lines.push(`| CPA | ${targets.cpa} kr | ${actual.cpa.toFixed(0)} kr | ${actual.cpa <= targets.cpa ? "✅" : "⚠️"} |`);
            if (targets.cpm)
                lines.push(`| CPM | ${targets.cpm} kr | ${actual.cpm.toFixed(0)} kr | ${actual.cpm <= targets.cpm ? "✅" : "⚠️"} |`);
            // Funnel-targets
            if (targets.funnel) {
                lines.push(``, `### Funnel-targets`);
                for (const [stage, stageTargets] of Object.entries(targets.funnel)) {
    ...
    server.tool("get_google_performance", "Hent aggregeret Google Ads performance for en klient (spend, conversions, ROAS, CTR). Querier insights-tabellen direkte for source=google_ads.", {
        client_name: z.string().describe("Klientens navn"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
    }, async ({ client_name, time_range }) => {
        const sb = getSupabase();
        ...
        if (error)
            return err(error.message);
        if (!data?.length)
            return noData(client.name, "Google Ads");
        ...
        if (impShareCount > 0) {
            ...
            lines.push(``, `**Search Impression Share (gns.)**`, `| Metric | Værdi |`, `|--------|-------|`, `| Impression Share | ${avgIS}% |`, `| Top Impression Share | ${avgTopIS}% |`, `| Tabt pga. budget | ${avgBudgetLost}% |`, `| Tabt pga. ranking | ${avgRankLost}% |`);
        }
        return text(lines.join("\n"));
    });
    // ─── Tool: get_google_campaigns ───────────────────────────────────────────
    server.tool("get_google_campaigns", "Hent Google Ads kampagner med aggregeret performance (spend, conversions, ROAS) fra insights-tabellen", {
        client_name: z.string().describe("Klientens navn"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
        ...
        let campQuery = sb
            ...
            .order("name");
        if (status !== "all")
            campQuery = campQuery.eq("status", status);
        const { data: campaigns, error: campError } = await campQuery;
        if (campError)
            return err(campError.message);
        if (!campaigns?.length)
            return text(`Ingen Google Ads kampagner fundet for ${client.name}`);
        const { since, until } = resolveDateRange(time_range);
        ...
        const withoutSpend = campaigns.filter((c) => {
            const m = campMetrics.get(c.id);
            return !m || m.spend === 0;
        });
        const lines = [
        ...
        if (withoutSpend.length > 0) {
            lines.push(``, `*${withoutSpend.length} kampagner uden spend i perioden (${withoutSpend.filter((c) => String(c.status) === "2").length} ENABLED)*`);
        }
        return text(lines.join("\n"));
    });
    // ─── Tool: get_channel_overview ────────────────────────────────────────────
    server.tool("get_channel_overview", "Cross-channel overblik: Meta Ads vs Google Ads (og Shopify revenue) side-by-side for en klient", {
        client_name: z.string().describe("Klientens navn"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
    }, async ({ client_name, time_range }) => {
        const sb = getSupabase();
        ...
        const shopify = (shopifyResult.data || []).reduce((acc, r) => ({
        ...
        function roas(revenue, spend) { return spend > 0 ? (revenue / spend).toFixed(2) + "x" : "–"; }
        function ctr(clicks, impressions) { return impressions > 0 ? ((clicks / impressions) * 100).toFixed(2) + "%" : "–"; }
        ...
        if (hasShopify) {
            lines.push(``, `### Shopify Revenue`, `| Metric | Værdi |`, `|--------|-------|`, `| Ordrer | ${formatNum(shopify.orders)} |`, `| Brutto-omsætning | ${formatCurrency(shopify.gross)} |`, `| Netto-omsætning | ${formatCurrency(shopify.net)} |`, `| Blended ROAS | ${totalSpend > 0 ? (shopify.net / totalSpend).toFixed(2) + "x" : "–"} |`);
        }
    ...
    server.tool("get_shopify_revenue", "Hent Shopify omsætningsdata (ordrer, brutto/netto revenue) med valgfri land-breakdown", {
        client_name: z.string().describe("Klientens navn"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
        ...
        if (error)
            return err(error.message);
        if (!data?.length)
            return noData(client.name, "Shopify");
        if (by_country) {
            ...
            const lines = [
                ...
                `|------|--------|--------|-------|---|------------|`,
                ...sorted.map(([cc, m]) => `| ${cc} | ${m.orders} | ${formatCurrency(m.gross)} | ${formatCurrency(m.net)} | ${totalGross > 0 ? (m.gross / totalGross * 100).toFixed(1) : "0"}% | ${m.newCustomers} |`),
            ];
            return text(lines.join("\n"));
        }
    ...
    server.tool("get_data_sources", "Vis hvilke datakilder der er tilsluttet for en klient (Meta, Google Ads, Klaviyo, Shopify) og deres sync-status", {
        client_name: z.string().describe("Klientens navn"),
    }, async ({ client_name }) => {
        const sb = getSupabase();
        const client = await findClient(sb, client_name);
        ...
        const googleDs = dataSources.filter((ds) => ds.source_type === "google_ads");
        const shopifyDs = dataSources.filter((ds) => ds.source_type === "shopify");
        const otherDs = dataSources.filter((ds) => !["google_ads", "shopify"].includes(ds.source_type));
    ...
    server.tool("get_google_keywords", "Hent Google Ads keyword-performance med Quality Score, predicted CTR, ad relevance og landing page experience. Sortér efter spend, QS eller conversions.", {
        client_name: z.string().describe("Klientens navn"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
        ...
        if (error)
            return err(error.message);
        if (!data?.length)
            return noData(client.name, "keyword");
        ...
        for (const r of data) {
            ...
            if (existing) {
                ...
                if (r.quality_score != null)
                    existing.qualityScore = Number(r.quality_score);
                if (r.search_predicted_ctr)
                    existing.predictedCtr = r.search_predicted_ctr;
                if (r.ad_relevance)
                    existing.adRelevance = r.ad_relevance;
                if (r.landing_page_experience)
                    existing.landingPage = r.landing_page_experience;
            }
            else {
                kwMap.set(key, {
        ...
        switch (sort_by) {
            case "spend":
                ...
                break;
            case "conversions":
                keywords.sort((a, b) => b.conversions - a.conversions);
                break;
            case "quality_score":
                keywords.sort((a, b) => (b.qualityScore || 0) - (a.qualityScore || 0));
                break;
            case "clicks":
    ...
    server.tool("get_google_search_terms", "Hent Google Ads søgeforespørgsler (search terms) der udløste annoncer. Vis spend, klik og conversions per søgeterm. Filtrer på status (ADDED/EXCLUDED/NONE).", {
        client_name: z.string().describe("Klientens navn"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
        ...
        if (error)
            return err(error.message);
        if (!data?.length)
            return noData(client.name, "search term");
        ...
        switch (sort_by) {
            case "spend":
                ...
                break;
            case "conversions":
                terms.sort((a, b) => b.conversions - a.conversions);
                break;
            case "clicks":
                terms.sort((a, b) => b.clicks - a.clicks);
                break;
        }
    ...
    server.tool("get_klaviyo_overview", "Klaviyo email-overblik: sendt, åbnet, klikket, orders, revenue + subscriber count", {
        client_name: z.string().describe("Klientens navn"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
    }, async ({ client_name, time_range }) => {
        const sb = getSupabase();
        ...
        let apiKey;
        try {
            apiKey = await resolveKlaviyoApiKey(sb, client.id);
        }
        catch {
    ...
    server.tool("get_klaviyo_flows", "Alle Klaviyo flows med performance: sendt, open%, click%, revenue", {
        client_name: z.string().describe("Klientens navn"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
    }, async ({ client_name, time_range }) => {
        const sb = getSupabase();
        ...
        try {
            ...
            if (!flows.length)
                return text(`Ingen flows fundet for ${client.name}`);
            ...
            const lines = [
                ...
                `|------|--------|-------|-------|--------|---------|-----------|`,
                ...sorted.map((f) => `| ${f.name} | ${f.status} | ${formatNum(f.received)} | ${f.openRate.toFixed(1)}% | ${f.clickRate.toFixed(1)}% | ${formatCurrency(f.revenue)} | ${f.revenuePerEmail.toFixed(1)} kr |`),
                ``,
    ...
    server.tool("get_klaviyo_campaigns", "Klaviyo campaigns med performance, sorteret efter seneste send", {
        client_name: z.string().describe("Klientens navn"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
        ...
        try {
            ...
            if (!campaigns.length)
                return text(`Ingen campaigns fundet for ${client.name} i perioden`);
    ...
    server.tool("get_klaviyo_revenue", "Klaviyo revenue attribution: flows vs campaigns, top 5 af hver", {
        client_name: z.string().describe("Klientens navn"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
    }, async ({ client_name, time_range }) => {
        const sb = getSupabase();
    ...
    server.tool("get_klaviyo_lists", "Klaviyo subscriber-lister med profil-antal", {
        client_name: z.string().describe("Klientens navn"),
    }, async ({ client_name }) => {
        const sb = getSupabase();
        const client = await findClient(sb, client_name);
    ...
    server.tool("get_klaviyo_segments", "Klaviyo segmenter med status og profil-antal", {
        client_name: z.string().describe("Klientens navn"),
    }, async ({ client_name }) => {
        const sb = getSupabase();
        const client = await findClient(sb, client_name);
        ...
        try {
            ...
            const lines = [
                ...
                `|---------|-------|---------|----------|`,
                ...stats.segments.map((s) => `| ${s.name} | ${s.isActive ? "✓" : "–"} | ${s.isStarred ? "★" : "–"} | ${s.profileCount > 0 ? `~${formatNum(s.profileCount)}+` : "0"} |`),
            ];
            return text(lines.join("\n"));
        }
        catch (e) {
    ...
    server.tool("get_klaviyo_metrics", "Tilgængelige Klaviyo metrics/events i kontoen", {
        client_name: z.string().describe("Klientens navn"),
    }, async ({ client_name }) => {
        const sb = getSupabase();
        const client = await findClient(sb, client_name);
        ...
        try {
            ...
            const lines = [
                ...
                `|--------|-------------|----|`,
                ...metrics.map((m) => `| ${m.name} | ${m.integration?.name || "–"} | ${m.id} |`),
                ``,
    ...
    server.tool("get_klaviyo_health", "Check Klaviyo-forbindelse: valider API key og hent basis-info", {
        client_name: z.string().describe("Klientens navn"),
    }, async ({ client_name }) => {
        const sb = getSupabase();
        const client = await findClient(sb, client_name);
        ...
        try {
            ...
            if (!valid)
                return text(`❌ Klaviyo API key for ${client.name} er ugyldig`);
    ...
    server.tool("get_klaviyo_campaign_content", "Detaljeret kampagneindhold: subject line, preview text, afsender, links og performance. Brug denne til at analysere en specifik kampagnes indhold.", {
        client_name: z.string().describe("Klientens navn"),
        campaign_name: z.string().describe("Kampagnenavn (delvis match OK)"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
        ...
        if (!data || data.length === 0)
            return text(`Ingen campaigns matchede "${campaign_name}" for ${client.name} i perioden ${time_range}`);
    ...
    server.tool("get_klaviyo_stored_campaigns", "📊 ANALYSE: Klaviyo kampagner fra Supabase med tag-gruppering, subject lines, CTOR, revenue. Brug DENNE til analyse — ikke get_klaviyo_campaigns (som kalder API direkte).", {
        client_name: z.string().describe("Klientens navn"),
        ...
        if (!targetMonth) {
            const { data: latest } = await sb
                ...
                .limit(1);
            if (!latest?.length)
                return text(`Ingen Klaviyo kampagnedata i Supabase for ${client.name}. Kør sync via dashboard (/settings/data-sources → Klaviyo → Sync).`);
        ...
        if (dbErr)
            return err(`Supabase: ${dbErr.message}`);
        if (!data?.length)
            return text(`Ingen kampagner for ${client.name} i ${targetMonth}. Kør Klaviyo sync.`);
        // Tag-grouped summary
        ...
        const lines = [
            ...
            `|-----|-------|-------|---------|------|`,
            ...[...tagMap.entries()].sort((a, b) => b[1].revenue - a[1].revenue).map(([tag, v]) => `| ${tag} | ${v.count} | ${formatNum(v.received)} | ${formatCurrency(v.revenue)} | ${v.opened > 0 ? ((v.clicked / v.opened) * 100).toFixed(1) + "%" : "–"} |`),
            ``,
            `### Top kampagner`,
    ...
    server.tool("get_klaviyo_stored_flows", "📊 ANALYSE: Klaviyo flows fra Supabase med revenue share og MoM. Brug DENNE til analyse — ikke get_klaviyo_flows (som kalder API direkte).", {
        client_name: z.string().describe("Klientens navn"),
        month: z.string().optional().describe("Måned i YYYY-MM format. Default: seneste tilgængelige."),
    }, async ({ client_name, month }) => {
        const sb = getSupabase();
        ...
        if (!targetMonth) {
            const { data: latest } = await sb
                ...
                .limit(1);
            if (!latest?.length)
                return text(`Ingen Klaviyo flow-data i Supabase for ${client.name}. Kør sync via dashboard.`);
            targetMonth = latest[0].snapshot_month;
        }
        const { data, error: dbErr } = await sb
            .from("klaviyo_flow_snapshots")
        ...
        if (dbErr)
            return err(`Supabase: ${dbErr.message}`);
        if (!data?.length)
            return text(`Ingen flows for ${client.name} i ${targetMonth}. Kør Klaviyo sync.`);
        const totalRevenue = data.reduce((sum, f) => sum + (parseFloat(f.revenue) || 0), 0);
        const lines = [
            `## ${client.name} – Klaviyo Flows (${targetMonth}) [Supabase]`,
            ``,
            `| Flow | Status | Sendt | OR | CTOR | Revenue | Rev Share |`,
            `|------|--------|-------|----|----- |---------|-----------|`,
            ...data.map((f) => `| ${(f.flow_name || "–").slice(0, 35)} | ${f.status} | ${formatNum(f.received)} | ${parseFloat(f.open_rate)?.toFixed(1) || "–"}% | ${parseFloat(f.ctor)?.toFixed(1) || "–"}% | ${formatCurrency(parseFloat(f.revenue) || 0)} | ${parseFloat(f.revenue_share)?.toFixed(1) || "–"}% |`),
            ``,
            `**Total flow revenue:** ${formatCurrency(totalRevenue)}`,
    ...
    server.tool("get_klaviyo_monthly", "📊 ANALYSE: Klaviyo månedlige aggregater fra Supabase. Revenue, OR, CTOR, campaigns sendt. Flow vs campaign split. MoM trends.", {
        client_name: z.string().describe("Klientens navn"),
        ...
        if (dbErr)
            return err(`Supabase: ${dbErr.message}`);
        if (!data?.length)
            return text(`Ingen Klaviyo månedlige aggregater for ${client.name}. Kør sync via dashboard (/settings/data-sources → Klaviyo → Sync).`);
        // Check for sync issues (all revenue = 0)
        const allZeroRevenue = data.every((d) => !d.total_revenue || parseFloat(d.total_revenue) === 0);
        ...
        const lines = [
            ...
            `|-------|-----------|-------|---------|----|----- |-----------|`,
            ...data.reverse().map((d) => `| ${d.month} | ${d.campaigns_sent || "–"} | ${formatNum(d.total_received)} | ${formatCurrency(parseFloat(d.total_revenue) || 0)} | ${parseFloat(d.avg_open_rate)?.toFixed(1) || "–"}% | ${parseFloat(d.avg_ctor)?.toFixed(1) || "–"}% | ${parseFloat(d.avg_conversion_rate)?.toFixed(1) || "–"}% |`),
            ``,
    ...
    server.tool("get_meta_ad_accounts", "List alle Meta ad accounts som en klients access token har adgang til. Nyttigt til at se hvilke konti der er tilgængelige, og matche dem med klienter i systemet.", {
        ...
        try {
            ...
            if (!clientData?.meta_access_token)
                return err(`${client.name} har ingen Meta access token.`);
            const token = clientData.meta_access_token;
            // Fetch all ad accounts (paginate)
            const accounts = [];
            let url = `https://graph.facebook.com/v21.0/me/adaccounts?fields=id,name,account_status,currency,timezone_name,amount_spent&limit=100&access_token=${token}`;
            while (url && accounts.length < 200) {
                const resp = await fetch(url);
                const json = await resp.json();
                if (json.error)
                    return err(`Meta API: ${json.error.message}`);
                accounts.push(...(json.data || []));
                url = json.paging?.next || null;
            }
            if (!accounts.length)
                return text(`Ingen ad accounts fundet for ${client.name}'s token.`);
            ...
            ourClients?.forEach(c => { if (c.meta_ad_account_id)
                trackedAccounts.set(c.meta_ad_account_id, c.name); });
            const statusMap = { 1: "ACTIVE", 2: "DISABLED", 3: "UNSETTLED", 7: "PENDING_REVIEW", 8: "PENDING_SETTLEMENT", 9: "GRACE_PERIOD", 100: "PENDING_CLOSURE", 101: "CLOSED" };
            const lines = [
    ...
    server.tool("get_ad_insights", "Fleksibel insights-query: vælg level (ad/adset/campaign/account), source (meta/google_ads/all), og metrics. Aggregerer over tidsperiode. Erstatter Pipedreams get_insights.", {
        ...
        try {
            ...
            if (adset_id)
                query = query.eq("ad_set_id", adset_id);
            const { data, error: qErr } = await query;
            if (qErr)
                return err(qErr.message);
            if (!data?.length)
                return noData(client.name, "insights");
            ...
            for (const r of data) {
                ...
                if (level === "account")
                    ...
                    key = r.ad_set_id || r.campaign_id || "unknown";
                else
                    key = r.ad_id || r.ad_set_id || "unknown";
                if (!groups.has(key))
                    groups.set(key, { label: key, spend: 0, impressions: 0, clicks: 0, purchases: 0, revenue: 0, reach: 0, link_clicks: 0, video_views: 0, rows: 0 });
            ...
            const sorted = [...groups.values()].sort((a, b) => {
                if (sort_by === "roas")
                    return (b.revenue / (b.spend || 1)) - (a.revenue / (a.spend || 1));
                return b[sort_by] - a[sort_by];
            }).slice(0, limit);
            const lines = [`## ${client.name} – Insights (${level}) | ${since} → ${until}\n`];
            if (level === "account") {
                const a = sorted[0];
                const roas = a.spend > 0 ? (a.revenue / a.spend).toFixed(2) : "–";
                lines.push(`| Metric | Værdi |`, `|--------|-------|`, `| Spend | ${formatCurrency(a.spend)} |`, `| Impressions | ${formatNum(a.impressions)} |`, `| Clicks | ${formatNum(a.clicks)} |`, `| Purchases | ${formatNum(a.purchases)} |`, `| Revenue | ${formatCurrency(a.revenue)} |`, `| ROAS | ${roas}x |`, `| CTR | ${a.impressions > 0 ? ((a.clicks / a.impressions) * 100).toFixed(2) : "–"}% |`, `| CPA | ${a.purchases > 0 ? formatCurrency(a.spend / a.purchases) : "–"} |`);
            }
            else {
    ...
    server.tool("get_daily_trend", "Daglig trend: spend, impressions, clicks, purchases, revenue per dag. Perfekt til at spotte trends og anomalier.", {
        client_name: z.string().describe("Klientnavn (fuzzy match)"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
        source: z.enum(["meta", "google_ads", "all"]).default("all").describe("Datakilde"),
        ...
        try {
            ...
            for (const r of data) {
                const d = r.date;
                if (!days.has(d))
                    days.set(d, { spend: 0, impressions: 0, clicks: 0, purchases: 0, revenue: 0 });
                const g = days.get(d);
                g.spend += r.spend || 0;
                g.impressions += r.impressions || 0;
                g.clicks += r.clicks || 0;
    ...
    server.tool("get_age_gender_breakdown", "Alder/køn-breakdown: spend, impressions, clicks, purchases, revenue fordelt på aldersgrupper og køn.", {
        client_name: z.string().describe("Klientnavn (fuzzy match)"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
        ...
        try {
            ...
            if (qErr)
                return err(qErr.message);
            if (!data?.length)
                return noData(client.name, "demografisk");
            const groups = new Map();
            for (const r of data) {
                let key;
                if (group_by === "age")
                    key = r.age_range;
                else if (group_by === "gender")
                    key = r.gender;
                else
                    key = `${r.age_range} / ${r.gender}`;
                if (!groups.has(key))
                    groups.set(key, { spend: 0, impressions: 0, clicks: 0, purchases: 0, revenue: 0 });
                const g = groups.get(key);
                g.spend += r.spend || 0;
                g.impressions += r.impressions || 0;
                g.clicks += r.clicks || 0;
    ...
    server.tool("get_placement_breakdown", "Platform/placement/device-breakdown: spend-fordeling på tværs af Facebook, Instagram, Audience Network, Messenger + placements (feed, stories, reels).", {
        client_name: z.string().describe("Klientnavn (fuzzy match)"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
        ...
        try {
            ...
            if (qErr)
                return err(qErr.message);
            if (!data?.length)
                return noData(client.name, "placement");
            const groups = new Map();
            for (const r of data) {
                let key;
                if (group_by === "platform")
                    key = r.platform || "unknown";
                else if (group_by === "placement")
                    key = `${r.platform}/${r.placement}` || "unknown";
                else
                    key = r.device_platform || "unknown";
                if (!groups.has(key))
                    groups.set(key, { spend: 0, impressions: 0, clicks: 0 });
                const g = groups.get(key);
                g.spend += r.spend || 0;
                g.impressions += r.impressions || 0;
                g.clicks += r.clicks || 0;
    ...
    server.tool("get_ad_details", "Detaljer for specifikke ads: navn, status, creative (headline, body, CTA, thumbnail), og performance. Søg på ad-navn eller ID.", {
        client_name: z.string().describe("Klientnavn (fuzzy match)"),
        ...
        try {
            ...
            if (qErr)
                return err(qErr.message);
            if (!ads?.length)
                return text(`Ingen ads fundet for ${client.name}`);
            ...
            if (search) {
                ...
                if (!filtered.length)
                    return text(`Ingen ads matcher "${search}" for ${client.name}`);
            }
            ...
            const lines = [`## ${client.name} – Ad Details\n`];
            for (const ad of filtered) {
                ...
                if (cr) {
                    ...
                    if (cr.body)
                        lines.push(`- **Body:** ${cr.body.length > 200 ? cr.body.slice(0, 197) + "…" : cr.body}`);
                    if (cr.link_url)
                        lines.push(`- **URL:** ${cr.link_url}`);
                    if (cr.thumbnail_url)
                        lines.push(`- **Thumbnail:** ${cr.thumbnail_url}`);
                }
                lines.push("");
            }
    ...
    server.tool("get_cross_client_overview", "Performance-overblik på tværs af ALLE klienter: spend, revenue, ROAS, purchases. Perfekt til agency-level overview.", {
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
        source: z.enum(["meta", "google_ads", "all"]).default("all").describe("Datakilde"),
        ...
        try {
            ...
            if (!clients?.length)
                return text("Ingen aktive klienter fundet.");
    ...
    server.tool("get_ad_image", "Hent creative thumbnail/billede for en specifik ad. Returnerer URL til billedet fra Supabase Storage.", {
        client_name: z.string().describe("Klientnavn (fuzzy match)"),
        ...
        try {
            ...
            if (creative_id) {
                ...
                if (!data?.length)
                    return text(`Creative ${creative_id} ikke fundet.`);
                const c = data[0];
            ...
            if (qErr)
                return err(qErr.message);
            if (!creatives?.length)
                return text(`Ingen creatives for ${client.name}`);
            ...
            if (!filtered.length)
                filtered = creatives.slice(0, maxImages);
            else
                filtered = filtered.slice(0, maxImages);
            const lines = [`## ${client.name} – Creative billeder\n`];
    ...
    server.tool("setup_assistant", "Installer NR Assistant (skills, guides, MCP config) på denne computer. Returnerer et setup-script som Claude Code kører lokalt.", {}, async () => {
...
**Opdatering:** Kør \`setup_assistant\` igen for at hente seneste version.`,
    ...
    server.tool("update_assistant", "Opdater NR Assistant til nyeste version (git pull + skill-kopi). Kør dette jævnligt for at få nye skills og forbedringer.", {}, async () => {
        const script = `#!/bin/bash
set -e
...
**Instruktion til Claude Code:** Kør ovenstående bash-script for at opdatere NR Assistant. Scriptet gitter pull fra GitHub og kopierer opdaterede skills til ~/.claude/skills/.`,
    ...
    server.tool("create_client", "Opret en ny klient i dashboardet. Kopierer Meta access token fra en eksisterende klient (da flere konti deler samme token). Bruges typisk efter get_meta_ad_accounts viser en utracked konto.", {
        ...
        try {
            ...
            if (existing?.length)
                return err(`Slug "${slug}" er allerede i brug. Vælg et andet navn.`);
            ...
            if (existingAcc?.length)
                return err(`Ad account ${meta_ad_account_id} er allerede tracked under "${existingAcc[0].name}".`);
            // Get token from source client
            const sourceClient = await findClient(sb, token_from_client);
            if (!sourceClient)
                return err(`Kilde-klient "${token_from_client}" ikke fundet — kan ikke kopiere access token.`);
            const { data: sourceData } = await sb.from("clients").select("meta_access_token").eq("id", sourceClient.id).single();
            if (!sourceData?.meta_access_token)
                return err(`${sourceClient.name} har ingen Meta access token at kopiere.`);
            // Insert new client
            ...
            if (error)
                return err(`Kunne ikke oprette klient: ${error.message}`);
            const lines = [
                `## Klient oprettet\n`,
                `| Felt | Værdi |`,
                `|------|-------|`,
    ...
    server.tool("trigger_backfill", "Start historisk Meta Ads backfill-sync via dashboardets Inngest-funktion. Standard: 2 år (730 dage). Synkroniserer insights, campaigns, ads, creatives, demographics, placements. Tager ca. 30-45 min for 2 år.", {
        client_name: z.string().describe("Klientnavn (fuzzy match)"),
        ...
        try {
            const dashboardUrl = process.env.DASHBOARD_URL;
            if (!dashboardUrl)
                return err("DASHBOARD_URL env var ikke sat — kan ikke trigge backfill.");
            const sb = getSupabase();
            const client = await findClient(sb, client_name);
            if (!client)
                return noClient(client_name);
            ...
            if (!clientData?.meta_ad_account_id)
                return err(`${client.name} har ingen Meta ad account ID. Opret klienten med create_client først.`);
            if (!clientData?.meta_access_token)
                return err(`${client.name} har ingen Meta access token.`);
    ...
    server.tool("get_google_ad_accounts", "List alle Google Ads konti tilgængelige via MCC (Manager Account). Viser hvilke der allerede er forbundet til klienter i systemet. Bruges til at opdage nye konti der kan tilsluttes.", {}, async () => {
        try {
            const data = await dashboardFetch("/api/connectors/google-ads/discover");
            const accounts = data.accounts || [];
            if (!accounts.length)
                return text("Ingen Google Ads konti fundet under MCC. Tjek at GOOGLE_ADS_MCC_ID og GOOGLE_ADS_REFRESH_TOKEN er sat korrekt.");
            ...
            const sorted = accounts.sort((a, b) => {
                if (a.already_connected !== b.already_connected)
                    return a.already_connected ? -1 : 1;
                return a.name.localeCompare(b.name);
            });
            let connectedCount = 0;
            for (const a of sorted) {
                if (a.already_connected)
                    connectedCount++;
    ...
    server.tool("connect_google_ads", "Forbind en Google Ads konto til en klient. Indsætter i data_sources-tabellen. Klienten skal eksistere i forvejen (opret med create_client hvis nødvendigt). Spørg altid om bekræftelse — Google Ads kontonavne kan ligne hinanden.", {
        ...
        try {
            ...
            if (error)
                return err(`Kunne ikke oprette data source: ${error.message}`);
            const lines = [
    ...
    server.tool("read_client_report", "Læs en rapport-fil fra Supabase Storage (client-reports bucket). Filer er markdown-rapporter gemt som f.eks. 'client-reports/{klient}/rapport-navn.md'. Brug list_client_reports for at se tilgængelige filer.", {
        ...
        try {
            ...
            if (error)
                return err(`Kunne ikke hente fil: ${error.message}`);
    ...
    server.tool("list_client_reports", "List rapport-filer i Supabase Storage (client-reports bucket). Angiv en klient-mappe for at se filer, eller udelad for at se alle mapper.", {
        ...
        try {
            ...
            if (error)
                return err(`Kunne ikke liste filer: ${error.message}`);
            if (!data?.length)
                return text(`Ingen filer i ${folder || "client-reports/"}`);
            const lines = [
    ...
    server.tool("write_client_report", "Skriv eller opdatér en rapport-fil i Supabase Storage (client-reports bucket). Brug til at gemme markdown-rapporter, analyser, eller reviews. Filer gemmes som '{klient}/{filnavn}.md'.", {
        ...
        try {
            ...
            if (error)
                return err(`Kunne ikke gemme fil: ${error.message}`);
    ...
    server.tool("get_google_shopping", "Hent Google Shopping produkt-performance (spend, clicks, ROAS) fra google_shopping_insights-tabellen", {
        client_name: z.string().describe("Klientens navn"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
        ...
        let query = sb
            ...
            .lte("date", until);
        if (brand)
            query = query.ilike("product_brand", `%${brand}%`);
        if (category)
            query = query.ilike("product_type_l1", `%${category}%`);
        const { data, error } = await query;
        if (error)
            return err(error.message);
        if (!data?.length)
            return noData(client.name, "Google Shopping");
    ...
    // ─── Tool: get_google_geo ──────────────────────────────────────────────────
    server.tool("get_google_geo", "Hent Google Ads geografisk performance pr. land fra google_geo_insights-tabellen", {
        client_name: z.string().describe("Klientens navn"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
    }, async ({ client_name, time_range }) => {
        const sb = getSupabase();
        ...
        if (error)
            return err(error.message);
        if (!data?.length)
            return noData(client.name, "Google Ads geo");
    ...
    server.tool("get_google_ad_groups", "Hent Google Ads ad group performance med spend, ROAS, conversions. Kan filtreres per kampagne.", {
        client_name: z.string().describe("Klientens navn"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
        ...
        if (asErr)
            return err(asErr.message);
        if (!adSets?.length)
            return text(`Ingen Google Ads ad groups fundet for ${client.name}`);
        ...
        if (campaign_name) {
            const matchingCampIds = new Set((campaigns || []).filter((c) => c.name.toLowerCase().includes(campaign_name.toLowerCase())).map((c) => c.id));
            filteredAdSets = adSets.filter((as) => matchingCampIds.has(as.campaign_id));
            if (!filteredAdSets.length)
                return text(`Ingen ad groups matcher kampagne "${campaign_name}"`);
        }
        // Get insights
        ...
        const withSpend = filteredAdSets.filter((as) => {
    ...
    server.tool("get_google_assets", "Hent Google Ads Performance Max asset groups og assets. Viser grupper med performance-score og tilknyttede assets.", {
        client_name: z.string().describe("Klientens navn"),
        ...
        if (gErr)
            return err(gErr.message);
        if (!groups?.length)
            return text(`Ingen PMax asset groups fundet for ${client.name}`);
        // Get campaigns for name lookup
        const campIds = [...new Set(groups.map((g) => g.campaign_id).filter(Boolean))];
        let campNameMap = new Map();
        if (campIds.length) {
            const { data: campaigns } = await sb.from("campaigns").select("id, name").in("id", campIds);
            campNameMap = new Map((campaigns || []).map((c) => [c.id, c.name]));
        }
        // Optional filter
        let filtered = groups;
        if (campaign_name) {
            const matchIds = new Set([...campNameMap.entries()].filter(([, n]) => n.toLowerCase().includes(campaign_name.toLowerCase())).map(([id]) => id));
            filtered = groups.filter((g) => matchIds.has(g.campaign_id));
        }
        // Get assets
        ...
        for (const g of filtered.slice(0, 15)) {
            ...
            if (groupAssets.length) {
                ...
                for (const a of groupAssets)
                    byType.set(a.field_type || a.asset_type || "?", (byType.get(a.field_type || a.asset_type || "?") || 0) + 1);
    ...
    server.tool("get_google_monthly_comparison", "MoM og YoY sammenligning af Google Ads performance. Viser aktuel, forrige måned og samme måned sidste år.", {
        client_name: z.string().describe("Klientens navn"),
        ...
        if (segment !== "all") {
            ...
            const { data: camps } = await sb
                ...
                .eq("source", "google_ads")
                .in("objective", types);
            campaignIdFilter = (camps || []).map((c) => c.id);
            if (!campaignIdFilter.length)
                return text(`Ingen ${segment} kampagner fundet for ${client.name}`);
        }
        ...
        async function fetchPeriod(start, end) {
            let q = sb
                ...
                .lte("date", end);
            if (campaignIdFilter)
                q = q.in("campaign_id", campaignIdFilter);
            const { data } = await q;
    ...
    server.tool("get_lead_campaign_breakdown", "Hent lead-kampagne breakdown med ROAS, leads, revenue og konverteringsdata pr. kampagne/ad set. Understøtter country filter.", {
        client_name: z.string().describe("Klientens navn"),
        ...
        try {
            ...
            const data = await dashboardFetch("/api/lead-cohorts/campaign-breakdown", params);
            if (!data?.campaigns?.length)
                return noData(client.name, "lead campaign breakdown");
            const lines = [
    ...
    server.tool("get_lead_unmatched", "Hent leads der ikke er matchet til en ordre endnu (unmatched leads). Viser konverteringspotentiale.", {
        client_name: z.string().describe("Klientens navn"),
        time_range: z.string().default("last_90d").describe(TIME_RANGE_DESC),
        ...
        try {
            ...
            if (country)
                params.country = country;
            const data = await dashboardFetch("/api/lead-cohorts/unmatched", params);
            if (!data?.unmatched?.length)
                return text(`Ingen umatchede leads for ${client.name} i perioden.`);
            const lines = [
                ...
                `|-----|-------|-------|----------|---------|`,
                ...data.unmatched.slice(0, 25).map((u) => {
    ...
    server.tool("get_cross_channel", "Cross-channel overblik: Meta + Google Ads side om side med daglig sammenligning. Viser spend, ROAS, conversions for begge kanaler.", {
        client_name: z.string().describe("Klientens navn"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
    }, async ({ client_name, time_range }) => {
        try {
            ...
            if (!client)
                return noClient(client_name);
            const { since, until } = resolveDateRange(time_range);
            const data = await dashboardFetch("/api/dashboard/cross-channel", {
                clientId: client.id,
                from: since,
                to: until,
            });
    ...
    server.tool("get_monthly_insights", "12-måneders performance trend + YTD sammenligning (i år vs. sidste år). Viser spend, revenue, ROAS, purchases pr. måned.", {
        client_name: z.string().describe("Klientens navn"),
    }, async ({ client_name }) => {
        try {
            const sb = getSupabase();
            ...
            if (!client)
                return noClient(client_name);
            const data = await dashboardFetch("/api/dashboard/monthly-insights", {
                clientId: client.id,
            });
            if (!data?.months?.length)
                return noData(client.name, "monthly insights");
            const lines = [
    ...
    server.tool("trigger_thumbnail_refresh", "Trigger en manuel opdatering af creative thumbnails i Supabase Storage for en klient", {
        client_name: z.string().describe("Klientens navn"),
    }, async ({ client_name }) => {
        const dashboardUrl = process.env.DASHBOARD_URL;
        if (!dashboardUrl)
            return err("DASHBOARD_URL env var ikke sat");
        const sb = getSupabase();
        const client = await findClient(sb, client_name);
        if (!client)
            return noClient(client_name);
        try {
            ...
            if (!resp.ok)
                return err(`Thumbnail refresh fejlede: ${resp.status}`);
    ...
    server.tool("generate_ai_review", "Generer en AI-baseret performance review for en klient. Bruger Claude Haiku til at analysere data og skrive en opsummering.", {
        client_name: z.string().describe("Klientens navn"),
        time_range: z.string().default("last_30d").describe(TIME_RANGE_DESC),
    ...
    server.tool("check_data_source_health", "Tjek health status for en data source (Meta, Google Ads, Klaviyo, Shopify). Viser seneste sync, fejl og forbindelsesstatus.", {
        client_name: z.string().describe("Klientens navn"),
        ...
        if (error)
            return err(error.message);
        if (!data?.length)
            return text(`Ingen data sources fundet for ${client.name}${source_type !== "all" ? ` (${source_type})` : ""}`);
        const lines = [
    ...
    server.tool("trigger_source_sync", "Trigger sync for en specifik data source. Bruger Inngest til at starte baggrundssync.", {
        client_name: z.string().describe("Klientens navn"),
        ...
        const { data: sources } = await sb
            ...
            .limit(1);
        if (!sources?.length)
            return err(`Ingen aktiv ${source_type} data source for ${client.name}`);
        try {
            ...
            if (!resp.ok)
                return err(`Sync trigger fejlede: ${resp.status}`);
    ...
    server.prompt("elle-weekly", "Ugentlig performance-rapport. Henter data, sammenligner med forrige uge, evaluerer signaler, og leverer Slack-klar review + fuld rapport.", { client_name: z.string().describe("Klientens navn (eller 'all' for alle klienter)") }, async ({ client_name }) => ({
        messages: [{
                role: "user",
                content: {
                    type: "text",
...
Hvis "${client_name}" er "all": gentag for hver klient med blank linje imellem + statuslinje: ✅ stærke | ⚠️ opmærksomhed | 🔴 kritiske`
    ...
    server.prompt("elle-analyze", "Performance-analyse — fra rå data til indsigt og anbefaling. Brug til kampagne-analyse, attribution-sammenligning og performance-reviews.", { client_name: z.string().describe("Klientens navn"), focus: z.string().optional().describe("Specifikt fokus (fx 'kreativ', 'audience', 'budget')") }, async ({ client_name, focus }) => ({
        messages: [{
                role: "user",
                content: {
                    type: "text",
...
CPC: >15kr svagt | 10-15kr OK | 5-10kr godt | <5kr fremragende`
    ...
    server.prompt("elle-review", "Månedlig status — fra performance-data til komplet klient-rapport med Meta, Klaviyo, Google og anbefalinger.", { client_name: z.string().describe("Klientens navn"), month: z.string().optional().describe("Måned (fx '2026-02' eller 'last_month')") }, async ({ client_name, month }) => ({
        messages: [{
                role: "user",
                content: {
                    type: "text",
...
- Actions skal være HANDLINGSRETTEDE med specifik ejer`
    ...
    server.prompt("elle-audit", "Comprehensive multi-channel audit af en klient. Meta + Klaviyo + Google. Struktureret gennemgang med fund og anbefalinger.", { client_name: z.string().describe("Klientens navn") }, async ({ client_name }) => ({
        messages: [{
                role: "user",
                content: {
                    type: "text",
...
\`write_client_report\` med path="[slug]/audit-[YYYY-MM-DD].md"`
    ...
    server.prompt("elle-creative-test", "Identificér hook patterns fra top-performende ads og generer test-koncepter for nye creatives.", { client_name: z.string().describe("Klientens navn") }, async ({ client_name }) => ({
        messages: [{
                role: "user",
                content: {
                    type: "text",
...
\`write_client_report\` med path="[slug]/creative-test-[YYYY-MM-DD].md"`
    ...
    server.prompt("elle-brief", "Lav et kampagne-brief med strukturerede spørgsmål. Output: udfyldt brief template klar til eksekvering.", { client_name: z.string().describe("Klientens navn"), campaign_type: z.string().optional().describe("Kampagnetype (fx 'product launch', 'seasonal', 'always-on')") }, async ({ client_name, campaign_type }) => ({
        messages: [{
                role: "user",
                content: {
                    type: "text",
...
\`write_client_report\` med path="[slug]/brief-[kampagnenavn]-[YYYY-MM-DD].md"`
    ...
    server.prompt("elle-creative", "Lav et creative brief til det kreative team — klar til produktion. Baseret på data og brand context.", { client_name: z.string().describe("Klientens navn"), concept: z.string().optional().describe("Koncept eller kampagne at lave creatives til") }, async ({ client_name, concept }) => ({
        messages: [{
                role: "user",
                content: {
                    type: "text",
...
\`write_client_report\` med path="[slug]/creative-brief-[koncept]-[YYYY-MM-DD].md"`
    ...
    server.prompt("elle-discover", "Quick discovery af en potentiel klient. Hurtig research, fit-vurdering og muligheds-kort.", { company_name: z.string().describe("Virksomhedens navn"), website: z.string().optional().describe("Website URL") }, async ({ company_name, website }) => ({
        messages: [{
                role: "user",
                content: {
                    type: "text",
...
// ─── Helpers ─────────────────────────────────────────────────────────────────
async function findClient(sb, name) {
    const { data } = await sb
        .from("clients")
        .select("id, name")
        .ilike("name", `%${name}%`)
...
function noClient(name) {
    return { content: [{ type: "text", text: `Ingen klient fundet: "${name}"` }] };
}
function err(msg) {
    return { content: [{ type: "text", text: `Fejl: ${msg}` }] };
}
function text(t) {
    return { content: [{ type: "text", text: t }] };
}
/** Return a "no data" response with a backfill suggestion */
function noData(clientName, dataType) {
    const what = dataType ? `${dataType} ` : "";
    return text(`Ingen ${what}data for ${clientName} i perioden.\n\n` +
...
const GOOGLE_CAMPAIGN_STATUS = { "0": "UNSPECIFIED", "1": "UNKNOWN", "2": "ENABLED", "3": "PAUSED", "4": "REMOVED" };
const GOOGLE_MATCH_TYPE = { "0": "UNSPECIFIED", "2": "EXACT", "3": "PHRASE", "4": "BROAD", "6": "BROAD" };
const GOOGLE_CAMPAIGN_TYPE = { "2": "SEARCH", "3": "DISPLAY", "4": "SHOPPING", "6": "VIDEO", "8": "SMART", "9": "LOCAL", "10": "PMAX", "11": "LOCAL_SERVICES", "12": "DISCOVERY", "13": "APP", "14": "DEMAND_GEN" };
const GOOGLE_SEARCH_TERM_STATUS = { "0": "UNSPECIFIED", "2": "ADDED", "3": "EXCLUDED", "4": "ADDED_EXCLUDED", "5": "NONE" };
const GOOGLE_QS_RATING = { "2": "BELOW_AVERAGE", "3": "AVERAGE", "4": "ABOVE_AVERAGE" };
function googleEnum(map, val) {
    const s = String(val ?? "");
    return map[s] || s;
}
